...
function get_wordpress {
	...
	sudo tar xzf files/wordpress.tar.gz > /dev/null
	...
	if [[ " ${WP_THEMES[@]} " =~ "Twenty Eleven" ]]; then
		sudo unzip files/twentyeleven.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Fifteen" ]]; then
		sudo unzip files/twentyfifteen.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Fourteen" ]]; then
		sudo unzip files/twentyfourteen.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Nineteen" ]]; then
		sudo unzip files/twentynineteen.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Seventeen" ]]; then
		sudo unzip files/twentyseventeen.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Sixteen" ]]; then
		sudo unzip files/twentysixteen.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Ten" ]]; then
		sudo unzip files/twentyten.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Thirteen" ]]; then
		sudo unzip files/twentythirteen.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Twelve" ]]; then
		sudo unzip files/twentytwelve.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Twenty" ]]; then
		sudo unzip files/twentytwenty.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi
