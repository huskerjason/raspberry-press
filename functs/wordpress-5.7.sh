...
function get_wordpress {
	...
	sudo tar xzf files/wordpress-5.7.tar.gz > /dev/null
	...
	if [[ " ${WP_THEMES[@]} " =~ "Twenty Eleven" ]]; then
		sudo unzip files/twentyeleven.3.7.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Fifteen" ]]; then
		sudo unzip files/twentyfifteen.2.9.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Fourteen" ]]; then
		sudo unzip files/twentyfourteen.3.1.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Nineteen" ]]; then
		sudo unzip files/twentynineteen.2.0.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Seventeen" ]]; then
		sudo unzip files/twentyseventeen.2.6.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Sixteen" ]]; then
		sudo unzip files/twentysixteen.2.4.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Ten" ]]; then
		sudo unzip files/twentyten.3.3.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Thirteen" ]]; then
		sudo unzip files/twentythirteen.3.3.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi
	...
	if [[ " ${WP_THEMES[@]} " =~ "Twenty Twenty" ]]; then
		sudo unzip files/twentytwenty.1.7.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi

	if [[ " ${WP_THEMES[@]} " =~ "Twenty Twenty One" ]]; then
		sudo unzip files/twentytwentyone.1.2.zip -d /var/www/html/wp-content/themes/ > /dev/null
	fi
